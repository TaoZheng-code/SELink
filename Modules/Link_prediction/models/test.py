...
def make_batches(df):
    ...
    print(len(tg_batch1))
    return (
    torch.tensor(msg_batch1, dtype=torch.long),
    torch.tensor(code_batch1, dtype=torch.long),
    torch.tensor(desc_batch1, dtype=torch.long),
    torch.tensor(tg_batch1, dtype=torch.long),
    torch.tensor(issueid_batch1, dtype=torch.long),
    commitid_batch1
)
...
def get_retrieval(df):
    ...
    cols2 = ['hash','message_processed', 'changed_files', 'Diff_processed', 'codelist_processed']
    for idx in tqdm(issue_id_list[:1000]):
        ...
        for j in range(len(commit)):
            ...
            resulted_false_link = resulted_false_link.reset_index(drop=True)
            test_df = test_df.append(resulted_false_link, ignore_index=True)

    print(len(test_df))
    test_df.to_csv('../../../data/test/ignite_test.csv')
    return test_df
...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../../../data/merge/ignite_link.csv')
    ...
    # Test
    retr_res_path = '../../../data/res/res_ignite.csv'
    ...
    result_df.to_csv(csv_file, mode="a", index=False, header=not pd.io.common.file_exists(csv_file))
